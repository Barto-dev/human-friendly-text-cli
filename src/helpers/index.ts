import { hoursDescription, minutesDescription } from '../config';

export const capitalizeFirstLetter = (string: string) => {
  return string.charAt(0).toUpperCase() + string.slice(1);
};

export const getHumanReadableNumberValue = (value: string, variant: 'hours' | 'minutes'): string => {
  if (variant === 'hours') {
    return hoursDescription[value];
  }

  return minutesDescription[value];
};
...
export const getHumanFriendlyTime = (time: string) => {
  ...
  let humanReadableHours = getHumanReadableNumberValue(hours, 'hours');
  let humanReadableMinutes = getHumanReadableNumberValue(minutes, 'minutes');
